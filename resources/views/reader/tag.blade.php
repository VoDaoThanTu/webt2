...
@section('main_content')
    ...
    <div class="section-heading">Từ khóa xu hướng: #{{ $tag->name }}</div>
        ...
        <div class="article-row">
            ...
            <div>
                <div class="article-meta">
                    <span class="text-info">{{ $art->category->name ?? 'Chưa rõ' }}</span>
                    {{ $art->user->fullname ?? 'Ẩn danh' }}
                    {{ $art->date_posted }}
                </div>
    ...
    @empty
        <div class="text-center text-muted py-5 italic" style="background-color: #1E2640; border: 1px solid #2D3748; border-radius: 8px;">
            Chưa có bài viết nào được gắn thẻ từ khóa này
        </div>
    @endforelse
